...
public class QQEmoji {
    ...
    public static void init(Context context) {
        ...
        int[] codes = context.getResources().getIntArray(context.getResources().getIdentifier("qq_emoji_code", "array", context.getPackageName()));
        TypedArray array = context.getResources().obtainTypedArray(context.getResources().getIdentifier("qq_emoji_res", "array", context.getPackageName()));
        TypedArray strArray = context.getResources().obtainTypedArray(context.getResources().getIdentifier("qq_emoji_description", "array", context.getPackageName()));
        ...
        while (++i < codes.length) {
            QQEmoji.EmojiInfo emoji = new QQEmoji.EmojiInfo(codes[i], array.getResourceId(i, -1), strArray.getResourceId(i, -1));
    ...
    private static List<QQEmoji.EmojiInfo> sEmojiList;
    ...
    public static class EmojiImageSpan extends ReplacementSpan {
        ...
        public EmojiImageSpan(int codePoint, float textSize) {
            if (sEmojiMap != null && sEmojiMap.containsKey(codePoint)) {
                QQEmoji.EmojiInfo emojiInfo = sEmojiMap.get(codePoint);
                if (emojiInfo == null) {
                    return;
                }
    ...
    public static int getEmojiCode(int index) {
        QQEmoji.EmojiInfo info = sEmojiList.get(index);
        return info.code;
    }
    ...
    public static CharSequence ensure(String input) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (sEmojiMap.containsKey(codePoint)) {
                ssb.setSpan(new QQEmoji.EmojiImageSpan(codePoint), isSurrogatePair ? i - 1 : i, i + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    ...
    public static boolean isQQEmoji(String input) {
        ...
        if (sEmojiMap.containsKey(chars)) {
            return true;
        }
    ...
    public static void ensure(Spannable spannable, float textSize) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (sEmojiMap.containsKey(codePoint)) {
                spannable.setSpan(new QQEmoji.EmojiImageSpan(codePoint, textSize), isSurrogatePair ? i - 1 : i, i + 1, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
            }
        }
    }
    ...
    public static int etEmojiCode(int index) {
        QQEmoji.EmojiInfo info = sEmojiList.get(index);
        return info.code;
    }
    ...
    public static Drawable getEmojiDrawable(Context context, int index) {
        ...
        if (index >= 0 && index < sEmojiList.size()) {
            QQEmoji.EmojiInfo emoji = sEmojiList.get(index);
